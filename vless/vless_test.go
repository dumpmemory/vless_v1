...
func TestVLessUDP(t *testing.T) {
	...
	targetStruct_forFakeUDPServer := &proxy.TargetAddr{
		Name:  "127.0.0.1",
		Port:  thePort,
		IsUDP: true,
	}
	// 开始监听 Client End LocalServer
	listener, err := net.Listen("tcp", fakeServerEndLocalServer.Addr())
	if err != nil {
		t.Logf("can not listen on %v: %v", fakeServerEndLocalServer.Addr(), err)
		t.FailNow()
	}
	go func() {
		for {
			...
			go func() {
				...
				if err != nil {
					t.Logf("failed in handshake form %v: %v", fakeServerEndLocalServer.Addr(), err)
					t.Fail()
				}
	...
	rc, _ := net.Dial("tcp", fakeServerEndLocalServer.Addr())
	defer rc.Close()

	wrc, err := fakeClientEndRemoteClient.Handshake(rc, targetStruct_forFakeUDPServer)
	if err != nil {
		log.Printf("failed in handshake to %v: %v", fakeServerEndLocalServer.Addr(), err)
		t.FailNow()
	}
	wrc.Write(hellodata)
